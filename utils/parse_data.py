...
def generate_features(wd="data", n_eigen=3, output=""):
    ...
    while True:
        ...
        try:
            ali = AlignIO.read(
                os.path.join(wd, "sim_msa_%s.phy" % i), format="phylip-relaxed"
            )
            print_update(str(i))
        except:
            break
        eig_tbl = pd.read_csv(
            os.path.join(wd, "sim_eigenvectors_tree_%s.csv" % i), header=None
        )
        ...
        # parse labels
        ali_labels = (
            pd.read_csv(os.path.join(wd, "sim_rates_%s.csv" % i), header=None)
            .to_numpy()[:, 1]
            .flatten()
        )
    ...
    if len(label_set):
        ...
        print(
            "\nFeatures saved as: %slabel_set.npy" % output,
            "%sfeature_set.npy" % output,
        )
    else:
        print("didnt work")
...
def rnn_in_out_dictionaries_from_sim(
    sim_file=None,
    sim=None,
    log_rates=True,
    log_tree_len=True,
    ...
    if "sub_model" in output_list and sim["labels_smodel"] is not None:
        dict_outputs["sub_model"] = np_to_tf(
            pd.get_dummies(sim["labels_smodel"]).to_numpy()
        )
...
def parse_alignment_file(
    ...
    if include_tree_features:
        eigenvec = pca_from_ali(dna, tree_builder="nj")[
            "eigenvect"
        ]  # shape = (n_taxa, n_taxa)
    ...
    if run_phyml_estimation:
        ...
        r_ml_est, tl_ml_est = run_phyml(
            ali_file + ".phy", path_phyml=phyml_path, model=2, ncat=4
        )
    else:
        r_ml_est = None
        tl_ml_est = None

    info.append({"r_ml_est": r_ml_est, "tl_ml_est": tl_ml_est})
...
def parse_large_alignment_file(ali_file, batch_size, n_taxa, truncate=None):
    counter = 1
    features = []
    ...
    for i in range(0, predict_sites, batch_size):
        if len(features_ali) % 50 == 0:
            print_update(
                "Sites: %s / %s (%s)"
                % (i, predict_sites, np.round((100 * i / predict_sites), 2))
            )
    ...
    dict_inputs = {
        "sequence_data": np.array(features_ali),
    }
    print("\ndone.\n")
    return dict_inputs


def load_phylornn_npz(
    sim_file=None,
    sim=None,
    log_rates=True,
    log_tree_len=True,
    ...
    train_dataset = tf.data.Dataset.from_tensor_slices(
        (sim["features_ali"], sim["labels_rates"], sim["labels_tl"])
    ).batch(5)

    return sim, train_dataset


def parse_alignment_file_gaps3D(
    ali_file, schema="fasta", drop_gaps=False, channel_dict=None
):
    # read fasta file
    dna = dendropy.DnaCharacterMatrix.get(file=open(ali_file), schema=schema)
    ...
    ali_3d = np.array([np.array(ali == i).astype(int) for i in channel_dict])
