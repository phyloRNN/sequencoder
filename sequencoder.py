...
MODEL_PATH = os.path.join(RESULT_DIR, "y_invariant_encoder_decorr_attention.pth")
...
class YInvariantAutoencoder128groupnorm(nn.Module):
    def __init__(self, latent_dim=128):
        ...
        self.fc_encode = nn.Sequential(
            nn.Linear(128, 256), nn.LeakyReLU(0.2), nn.Linear(256, latent_dim)
        )

        # --- DECODER ---
        self.fc_decode = nn.Sequential(
            nn.Linear(latent_dim, 256), nn.LeakyReLU(0.2), nn.Linear(256, 128)
        )
    ...
    def forward(self, x):
        ...
        x_recon = torch.nn.functional.interpolate(
            x_recon, size=(x.shape[2], x.shape[3]), mode="bilinear"
        )
...
if __name__ == "__main__":
    ...
    if TRAIN:
        ...
        for epoch in range(EPOCHS):
            ...
            for batch_n, batch in enumerate(train_loader, 1):
                ...
                total_batch_loss = (recon_loss / virtual_batch_size) + (
                        d_loss * lambda_decorr / virtual_batch_size
                )
                total_batch_loss.backward()

                # 5. Buffer Management
                with torch.no_grad():
                    running_mu = momentum * running_mu + (
                            1 - momentum
                    ) * latent.detach().mean(dim=0)
                ...
                if batch_n % virtual_batch_size == 0 or batch_n == len(train_loader):
                    ...
                    s = (
                            s
                            + f" D-Loss: {d_loss.item():.3f} Max/Avg Corr: {max_c:.2f} {mean_corr:.2f}"
                    )
    ...
    train_loader = DataLoader(
        dataset, batch_size=BATCH_SIZE, collate_fn=variable_collate_fn
    )
    ...
    if plot_model:
        ...
        model_graph.visual_graph.render(
            format="pdf", directory=MODEL_PATH, cleanup=True
        )
    ...
    if plot_attention_map:
        ...
        for _ in range(num_samples):
            try:
                ...
                attn_norm = (
                    F.softmax(raw_attn.view(b, 1, -1), dim=2).view(h, w).cpu().numpy()
                )
        ...
        for i, attn_map in enumerate(plot_data):
            im = axes[i].imshow(
                attn_map, aspect="auto", cmap="magma"
            )  # 'magma' is often clearer for small peaks
